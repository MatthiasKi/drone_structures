...
class TDLDRApproximator:
    ...
    def benchmark_approximate(self, target_mat: np.ndarray, param_share: float, log_filepath: str):
        assert len(target_mat.shape) == 2, "Can only handle matrices for LDR approximation"

        best_ldr_mat = None
        best_approx_mat_dense = np.zeros_like(target_mat)
        best_norm_difference = np.linalg.norm(target_mat, ord=self.norm)
        nb_parameters = 0
        ...
        if target_mat.shape[0] == target_mat.shape[1]:
            ...
            displacement_rank = self.get_max_ld_rank_wrt_max_nb_free_parameters(max_nb_free_parameters=max_nb_free_parameters, target_shape_mat=target_mat.shape)
            if displacement_rank > 0:
                nb_parameters = self.get_nb_free_parameters(displacement_rank, target_mat.shape)

                # We repeat the optimization 5 times to account for random initialization effects
                for _ in range(5):
                    ldr_mat, approx_mat_dense = self.square_mat_ldr_approximation_torch(weights=target_mat, displacement_rank=displacement_rank, verbose=False)
                
                    norm = np.linalg.norm(target_mat - approx_mat_dense, self.norm)
...
if __name__ == "__main__":
    ...
    for norm in test_norms:
        approximator = TDLDRApproximator(norm=norm)
        ...
        print("Norm Difference: " + str(norm_difference) + " (Original Mat Norm: " + str(np.linalg.norm(target_mat, ord=norm)))
